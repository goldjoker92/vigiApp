...
function isExpo(t) {
  return !!t && /^ExponentPushToken\[[\w\-]+\]$/.test(t);
}
function isFcm(t) {
  return !!t && t.includes(':APA91') && t.length > 80;
}

exports.registerDeviceMissing = onRequest(
    ...
    try {
      ...
      if (payload.fcmToken && tset.length) {
        for (const tile of tset) {
          ...
          try {
            await admin.messaging().subscribeToTopic(payload.fcmToken, topic);
            subscribed.push(topic);
          } catch (e) {
            console.warn(NS, 'sub_fail', topic, e?.message || e);
          }
        }
      }

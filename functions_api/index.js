...
app.get("/health", (_req, res) => res.json({ ok:true, service:"api", region:"southamerica-east1" }));
...
app.post("/signup", async (req, res) => {
  try {
    ...
    await ref.set({ email, storedHash:rec.storedHash, perUserSalt:rec.perUserSalt, hashVersion:rec.hashVersion, createdAt:new Date().toISOString() });
...
app.post("/login", async (req, res) => {
  try {
    ...
    const effectivePrehash = (prehash && saltId && Object.hasOwn(HASH_SCHEMES, saltId))
      ? prehash
      : (typeof password === "string" ? sha256HexServerPrefixed(password) : null);
    if (!effectivePrehash) return res.status(400).json({ ok:false, code:"MISSING_CREDENTIALS" });

    const snap = await usersCol().doc(email).get();
    if (!snap.exists) return res.status(401).json({ ok:false });

    const user = snap.data();
    const ok = await verifyFromClientPrehash(effectivePrehash, { storedHash:user.storedHash, perUserSalt:user.perUserSalt, hashVersion:user.hashVersion });

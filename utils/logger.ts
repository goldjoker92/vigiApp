...
export function setLogLevel(l: Level) { GLOBAL_LEVEL = l; }

const levelRank: Record<Level, number> = {
  silent: 99, error: 0, warn: 1, info: 2, debug: 3, verbose: 4,
};
...
export function createLogger(ns: string, opts?: {level?: Level; throttleMs?: number}) {
  ...
  function hash(parts: any[]) {
    try { return JSON.stringify(parts); } catch { return String(parts); }
  }

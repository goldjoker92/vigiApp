...
export async function cacheSet(key, value, ttlSec = 3600) {
  ...
  try { await AsyncStorage.setItem(key, j({ value, expiresAt })); } catch {}
}
...
export async function cacheGet(key) {
  try {
    ...
    if (!expiresAt || Date.now() > expiresAt) { AsyncStorage.removeItem(key).catch(()=>{}); return null; }
    return value;
  } catch { return null; }
}
...
export async function cacheSetForever(key, value) {
  try { await AsyncStorage.setItem(key, j({ value, expiresAt: Date.now() + 100*365*24*3600*1000 })); } catch {}
}
// get sans TTL
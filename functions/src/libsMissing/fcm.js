...
const TTL_CREATED = 3600,
  TTL_RESOLVED = 1800;

function android(ttl) {
  return { priority: 'high', ttl: ttl * 1000, notification: { channelId: 'missing_alerts' } };
}
function apns(ttl) {
  return {
    headers: { 'apns-expiration': String(Math.floor(Date.now() / 1000) + ttl) },
    payload: { aps: { category: 'missing_alerts', sound: 'default' } },
  };
}
...
async function sendToTopic(topic, payload, { ttl = TTL_CREATED } = {}) {
  ...
  console.log(NS, 'topic_ok', { topic, id });
  return id;
}

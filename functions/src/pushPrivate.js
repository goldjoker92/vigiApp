...
const { functions, db, assertRole, getTokensByUserIds, createDeliveryLog, expoPushSend } = require("./utils");
...
exports.sendPrivateAlertByGroup = functions.https.onCall(async (data, context) => {
  ...
  console.log("[sendPrivateAlertByGroup] done", { groupId, count: tokens.length, logId: logRef.id });

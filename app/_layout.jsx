...
function log(...a) {
  if (__DEV__ || !SILENCE_RELEASE) {
    try { console.log(\`[\${APP_TAG}][\${LAYOUT_TAG}][\${ts()}]\`, ...a); } catch {}
  }
}
function warn(...a) {
  if (__DEV__ || !SILENCE_RELEASE) {
    try { console.warn(\`[\${APP_TAG}][\${LAYOUT_TAG}][\${ts()}]\`, ...a); } catch {}
  }
}
function err(...a) {
  try { console.error(\`[\${APP_TAG}][\${LAYOUT_TAG}][\${ts()}]\`, ...a); } catch {}
}
...
function safeJson(obj) {
  try { return JSON.stringify(obj).slice(0, 1000); } catch { return "[unserializable]"; }
}
...
function PushBootstrap() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try { wireAuthGateForNotifications(auth); }
      catch (e) { warn("wireAuthGateForNotifications error:", (e && e.message) || e); }

      // Android : canaux D’ABORD, puis init (perms, cold start)
      try { await ensureAndroidChannels(); }
      catch (e) { warn("ensureAndroidChannels error:", (e && e.message) || e); }

      try { await initNotifications(); log("initNotifications ok"); }
      catch (e) { warn("initNotifications error:", (e && e.message) || e); }
      ...
      try {
        const maybeDetach = attachNotificationListeners({
          onReceive: (n) => {
            ...
            const sev = (content.data && (content.data.severidade || content.data.severity)) || undefined;
          ...
          onResponse: (r) => {
            const data = r && r.notification && r.notification.request && r.notification.request.content && r.notification.request.content.data;
    ...
    return () => {
      log("unmount → cleanup…");
      try { detachListeners && detachListeners(); log("listeners detached"); }
      catch (e) { err("detach listeners error:", (e && e.message) || e); }
    };
  }, []); // montage unique
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const unsub = onAuthStateChanged(auth, async (fbUser) => {
          if (!fbUser) { log("auth: signed out (no upsert)"); return; }
          ...
          try {
            const res = await upsertDevice({
              ...
              cep,
              appVersion: Application.nativeApplicationVersion || Application.applicationVersion || "0",
    ...
    return () => {
      try { unsubscribeAuth && unsubscribeAuth(); log("auth listener detached (upsert)"); }
      catch (e) { err("detach auth error:", (e && e.message) || e); }
    };
  }, [user && user.cep]);

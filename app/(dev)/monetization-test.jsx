...
} from '../../src/monetization/interstitial';
...
export default function MonetizationTest() {
  ...
  const buy = async (pkg) => {
    ...
    try {
      ...
      setMsg(`Achat annulé/erreur: ${e?.message || e}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      <Pressable
        onPress={() => showInterstitialIfAllowed()}
          ...
          {pkgs.map((p) => (
            ...
            >
              ...
              {!!p.product.description && (
                <Text style={{ color: '#f0f0f0', fontSize: 12 }}>{p.product.description}</Text>
              )}
            </Pressable>
          ))}
        </View>

...
const L = {
  ...
  step: (tid, step, extra = {}) =>
    console.log(NS, 'STEP', step, { traceId: tid, at: nowIso(), ...extra }),
};
...
const inBR = ({ lat, lng }) => {
  return (
    lat >= BR_BOUNDS.latMin &&
    lat <= BR_BOUNDS.latMax &&
    lng >= BR_BOUNDS.lngMin &&
    lng <= BR_BOUNDS.lngMax
  );
};
    ...
    deb.current = setTimeout(async () => {
      ...
      try {
        ...
        const resp = await fetch(url, {
          headers: { 'Accept-Language': 'pt-BR', 'User-Agent': 'VigiApp/OSM' },
        });
        ...
        const nice = mapped.map((m) => {
          ...
          return {
            ...m,
            labelShort: [rua, [cidade, uf].filter(Boolean).join(' / ')].filter(Boolean).join(' · '),
          };
        });
        setItems(nice);
...
const initialForm = {
  ...
  childDobBR: '',
  childSex: '',
  lastSeenDateBR: initialDateShort,
  lastSeenTime: '',
  lastRua: '',
  lastNumero: '',
  lastCidade: '',
...
const ensureCaseId = (id, dispatch) => {
  ...
  try {
    dispatch({ type: 'SET', key: 'caseId', value: nid });
  } catch {}
  return nid;
};
...
export default function MissingStart() {
  ...
  const onEditAddress = () => {
    L.step(traceIdRef.current, 'OSM/EDIT_REENABLE', {
      prevLocked: streetAuto.locked,
      prevPickedOnce: osmPickedOnce,
    });
  ...
  const cancelUpload = (k) => {
    try {
      aborters.current[k]?.abort();
      aborters.current[k] = null;
      setBusy(k, false);
      setPct(k, 0);
  ...
  async function pickFromLibrary(kind) {
    try {
      ...
      if (perm && !perm.granted) {
        show('Sem permissão para galeria');
        return null;
      }
      ...
      L.w('UPLOAD/PICK_ERR', e?.message || e);
  ...
  async function onUpload(kind) {
    if (uploading[kind]) {
      return cancelUpload(kind);
    }
    const picked = await pickFromLibrary(kind);
    if (!picked) {
      return;
    }
    const ensuredId = ensureCaseId(caseId, dispatch);
    const ctrl = new AbortController();
    aborters.current[kind] = ctrl;
    setPct(kind, 0);
    setBusy(kind, true);
    try {
      const common = {
      ...
      let out;
      if (kind === 'photo') {
        out = await uploadMainPhoto(common);
      } else if (kind === 'id_front') {
        out = await uploadIdFront(common);
      } else if (kind === 'id_back') {
        out = await uploadIdBack(common);
      } else if (kind === 'link_front') {
        out = await uploadLinkFront(common);
      } else if (kind === 'link_back') {
        out = await uploadLinkBack(common);
      }
      if (!out?.url) {
        L.w('UPLOAD/NO_URL', { kind });
      ...
      if (kind === 'photo') {
        dispatch({
          type: 'BULK_SET',
          payload: { photoPath: out.url, photoStoragePath: out.path, caseId: ensuredId },
        });
      }
      if (kind === 'id_front') {
        dispatch({
          type: 'BULK_SET',
          payload: { hasIdDocFront: true, idDocFrontPath: out.url, caseId: ensuredId },
        });
      }
      if (kind === 'id_back') {
        dispatch({
          type: 'BULK_SET',
          payload: { hasIdDocBack: true, idDocBackPath: out.url, caseId: ensuredId },
        });
      }
      if (kind === 'link_front') {
        dispatch({
          type: 'BULK_SET',
          payload: { hasLinkDocFront: true, linkDocFrontPath: out.url, caseId: ensuredId },
        });
      }
      if (kind === 'link_back') {
        dispatch({
          type: 'BULK_SET',
          payload: { hasLinkDocBack: true, linkDocBackPath: out.url, caseId: ensuredId },
        });
      }
      setPct(kind, 100);
    } catch (e) {
      if (e?.name !== 'AbortError') {
        L.e('UPLOAD/ERR', { kind, err: e?.message || e });
        show('Erro no upload.');
      }
    } finally {
      setBusy(kind, false);
      aborters.current[kind] = null;
      setTimeout(() => {
  ...
  const buildPayload = useCallback(() => {
    const p = {
      ...
      hasLinkDoc: !!(
        ...
        form.hasLinkDocBack ||
        form.linkDocFrontPath ||
        form.linkDocBackPath
      ),
      ...
      [
        {
          text: 'Cancelar',
          style: 'cancel',
          onPress: () => L.step(traceIdRef.current, 'ADDR/MANUAL_VALIDATE/CANCEL'),
        },
        {
          text: 'Usar minha posição',
          style: 'default',
          onPress: async () => {
            L.step(traceIdRef.current, 'ADDR/MANUAL_VALIDATE/CLICK');
            const dev = await captureGeolocationOnce(traceIdRef.current);
            if (!dev) {
              show('Sem localização do dispositivo.');
              L.w('ADDR/MANUAL_VALIDATE/NO_DEVICE');
              return;
            }
            const s = swapIfLooksSwapped({ lat: dev.lat, lng: dev.lng });
            if (s.swapped) {
              L.w('LOC/SANITY/SWAP_LATLNG', { before: dev, after: s });
            }
            if (!inBR(s)) {
              L.w('LOC/SANITY/OUT_OF_BOUNDS_DEVICE', s);
              show('Localização fora do Brasil.');
              return;
            }
            dispatch({
              type: 'BULK_SET',
            ...
            L.step(traceIdRef.current, 'ADDR/MANUAL_VALIDATE/OK', {
              source: 'gps',
              lat: s.lat,
              lng: s.lng,
            });
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      const deviceGeo = deviceGeoRaw
        ? swapIfLooksSwapped({ lat: toNum(deviceGeoRaw.lat), lng: toNum(deviceGeoRaw.lng) })
        : null;
      if (deviceGeo?.swapped) {
        L.w('LOC/SANITY/SWAP_LATLNG_DEVICE', { after: deviceGeo });
      }

      // 2) Construire "location" à partir de l'adresse (prioritaire) OU fallback
      let useLocation = null; // {lat,lng,source,addressConfidence}
      if (form.addrSource === 'address' && isFinite(form.addrLat) && isFinite(form.addrLng)) {
        const maybeSwap = swapIfLooksSwapped({
          lat: toNum(form.addrLat),
          lng: toNum(form.addrLng),
        });
        if (maybeSwap.swapped) {
          L.w('LOC/SANITY/SWAP_LATLNG_ADDR', { after: maybeSwap });
        }
        if (inBR(maybeSwap)) {
          useLocation = {
            lat: maybeSwap.lat,
            lng: maybeSwap.lng,
            source: 'address',
            addressConfidence: clamp(form.addrConfidence || 0.85, 0, 1),
          };
        } else {
      ...
      const lastSeenISO = form.lastSeenDateBR
        ? shortToISO(normalizeDateShort(form.lastSeenDateBR), form.lastSeenTime || '00:00')
        : null;
      ...
      const validated = {
        ...
        submitMeta: {
          geo: deviceGeo ? { lat: deviceGeo.lat, lng: deviceGeo.lng, t: Date.now() } : null,
          submittedAt: Timestamp.now(),
        },
        updatedAt: Timestamp.now(),
      };

      L.step(tid, 'PAYLOAD/LOCATION_SET', {
        source: validated.location.source,
        confidence: validated.location.addressConfidence,
      });
      ...
      L.step(tid, 'VERIFY/OK', {
        ownerId: data?.ownerId,
        kind: data?.kind,
        city: data?.lastKnownAddress?.cidade,
      });
  ...
  const ProgressInline = ({ kind }) => {
    ...
    return (
      <View style={styles.pWrap}>
        ...
        {up ? (
          <TouchableOpacity onPress={() => cancelUpload(kind)} style={styles.pCancel}>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
            ...
            {form.addressMode === 'osm' && (
              <View style={{ marginTop: 10 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                  ...
                  {form.addrSource === 'address' &&
                    isFinite(form.addrLat) &&
                    isFinite(form.addrLng) && (
                      <View style={styles.badgeOk}>
                        <Text style={styles.badgeOkTxt}>OSM ok</Text>
                      </View>
                    )}
                </View>
              </View>
            ...
            {form.addressMode === 'manual' && (
              <View style={{ marginTop: 10 }}>
                ...
                {form.addrSource === 'gps' && isFinite(form.addrLat) && isFinite(form.addrLng) && (
                  <View style={[styles.badgeWarn, { marginTop: 8 }]}>
                    <Text style={styles.badgeWarnTxt}>GPS aproximado</Text>
                  </View>
                )}
              </View>
            )}
            ...
            <View style={styles.card}>
              ...
              <View style={{ marginTop: 10 }}>
                <TouchableOpacity
                  style={[
                    styles.btnGhost,
                    form.hasIdDocFront && styles.btnOk,
                    uploading.id_front && styles.btnBusy,
                  ]}
                  onPress={() => onUpload('id_front')}
                >
                  <FileCheck2 color={form.hasIdDocFront ? '#22C55E' : '#7dd3fc'} size={16} />
                  <Text style={styles.btnTxt}>Documento (frente)</Text>
                </TouchableOpacity>
                ...
                {needsAdultBack && (
                  <>
                    <TouchableOpacity
                      style={[
                        styles.btnGhost,
                        form.hasIdDocBack && styles.btnOk,
                        uploading.id_back && styles.btnBusy,
                      ]}
                      onPress={() => onUpload('id_back')}
                    >
                      <FileCheck2 color={form.hasIdDocBack ? '#22C55E' : '#7dd3fc'} size={16} />
                      <Text style={styles.btnTxt}>Documento (verso)</Text>
                    </TouchableOpacity>
              ...
              {needsChildFront && (
                <>
                  <TouchableOpacity
                    style={[
                      styles.btnGhost,
                      form.hasLinkDocFront && styles.btnOk,
                      uploading.link_front && styles.btnBusy,
                    ]}
                    onPress={() => onUpload('link_front')}
                  >
                    <FileCheck2 color={form.hasLinkDocFront ? '#22C55E' : '#7dd3fc'} size={16} />
                    <Text style={styles.btnTxt}>Peça (frente)</Text>
                  </TouchableOpacity>
              ...
              {needsChildBack && (
                <>
                  <TouchableOpacity
                    style={[
                      styles.btnGhost,
                      form.hasLinkDocBack && styles.btnOk,
                      uploading.link_back && styles.btnBusy,
                    ]}
                    onPress={() => onUpload('link_back')}
                  >
                    <FileCheck2 color={form.hasLinkDocBack ? '#22C55E' : '#7dd3fc'} size={16} />
                    <Text style={styles.btnTxt}>Peça (verso)</Text>
                  </TouchableOpacity>
          ...
          <View style={styles.card}>
            <Text style={styles.cardTitle}>
              {type === 'animal' ? 'Animal' : type === 'object' ? 'Objeto' : 'Criança'}
            </Text>
            <Text style={styles.cardSubtitle}>
              {type === 'animal'
                ? 'Nome e sinais ajudam.'
                : type === 'object'
                  ? 'Ex.: iPhone 13, mochila preta…'
                  : 'Primeiro nome ajuda a circulação.'}
            </Text>
                ...
                <View style={styles.sexoRow}>
                  {['F', 'M'].map((s) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.chip,
                        s === 'F' ? styles.chipF : styles.chipM,
                        form.childSex === s && (s === 'F' ? styles.chipFOn : styles.chipMOn),
                      ]}
          ...
          <View style={styles.card}>
            ...
            <View style={{ marginTop: 10 }}>
              <TouchableOpacity
                style={[
                  styles.btnGhost,
                  form.photoPath && styles.btnOk,
                  uploading.photo && styles.btnBusy,
                ]}
...
const styles = StyleSheet.create({
  ...
  chipFOn: { backgroundColor: '#f472b6', borderColor: '#f472b6' },
  chipM: { backgroundColor: '#231a1a', borderWidth: 2, borderColor: '#ef4444' },
  chipMOn: { backgroundColor: '#ef4444', borderColor: '#ef4444' },
  ...
  btnOk: { borderColor: '#22C55E' },
  btnBusy: { opacity: 0.9, borderColor: '#3b82f6' },
  btnTxt: { color: '#cfd3db', fontWeight: '600' },
  ...
  pTxt: { textAlign: 'center', color: '#e5e7eb', fontSize: 11, lineHeight: 14, fontWeight: '700' },
  pCancel: { position: 'absolute', right: 6, top: -12, padding: 6 },

...
// -------------------------------------------------------------
import "../lib/firebase"; // ⚡ Assure l'init Firebase côté client
...
import { upsertDevice } from "../lib/registerDevice";
  ...
  async function onSaveDevice() {
    ...
    try {
      ...
      const res = await upsertDevice({
        ...
        expoPushToken: token,
        cep: cep.trim(),
        lat: null, lng: null,    // optionnels pour la V1 CEP
        geohash: null,
    ...
    <View style={[styles.container, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 12 }]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={[styles.card, { width: cardWidth }]}>
          ...
          <Text style={styles.subtitle}>Valide FCM ⇄ Expo ⇄ ton device. Zéro UI publique, juste DEV.</Text>

          {/* 1) Token */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>1) Autorisations & Token</Text>
            <Pressable style={[styles.btn, loading && styles.disabled]} onPress={onRegister} disabled={loading}>
              {loading ? <ActivityIndicator /> : <Text style={styles.btnText}>Obtenir le token</Text>}
            </Pressable>

            <View style={styles.tokenBox}>
              <Text style={styles.tokenLabel}>Expo push token</Text>
              <Text selectable style={styles.tokenValue}>{token || "— token non obtenu —"}</Text>
            </View>

            <Pressable style={[styles.btn, sending && styles.disabled]} onPress={onSendExpoTest} disabled={sending}>
              {sending ? <ActivityIndicator /> : <Text style={styles.btnText}>Envoyer un push test</Text>}
            </Pressable>
            <Text style={styles.hint}>Si rien n’arrive: permissions notifs + batterie (exclusion) + réseau OK.</Text>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.row}>
              <TextInput
                ...
                style={styles.input}
              />
              <Pressable style={[styles.btn, styles.btnInline, saving && styles.disabled]} onPress={onSaveDevice} disabled={saving}>
                {saving ? <ActivityIndicator /> : <Text style={styles.btnText}>Enregistrer</Text>}
              </Pressable>
            </View>
            <Text style={styles.hint}>Vérifie dans Firestore → devices que le doc existe (cep + token).</Text>
          </View>
          ...
          <Text style={styles.footer}>Une fois validé, appelle la Function “public = CEP” depuis la console Firebase.</Text>
        </View>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  card: {
    backgroundColor: palette.card, borderRadius: 16, padding: 16, marginVertical: 12, gap: 12,
    shadowColor: "#000", shadowOpacity: 0.25, shadowOffset: { width: 0, height: 8 }, shadowRadius: 16, elevation: 8,
  },
  ...
  tokenBox: { backgroundColor: "#11141A", borderRadius: 12, padding: 12, borderWidth: 1, borderColor: palette.ghostBorder, gap: 8 },
  tokenLabel: { color: palette.sub, fontSize: 12 },
  tokenValue: { color: palette.text, fontSize: 13 },
  row: { flexDirection: "row", gap: 10, alignItems: "center" },
  input: {
    flex: 1, backgroundColor: "#11141A", color: palette.text, borderRadius: 10,
    borderWidth: 1, borderColor: palette.ghostBorder, paddingHorizontal: 12, paddingVertical: 10,
  },
  btn: { backgroundColor: palette.primary, paddingVertical: 12, borderRadius: 12, alignItems: "center" },
  btnInline: { paddingHorizontal: 16 },
  btnText: { color: palette.primaryText, fontWeight: "700", fontSize: 16 },
  btnGhost: { flex: 1, borderWidth: 1, borderColor: palette.ghostBorder, paddingVertical: 10, borderRadius: 10, alignItems: "center" },
  btnGhostText: { color: palette.text, fontWeight: "600" },
  btnDanger: { backgroundColor: palette.danger, paddingVertical: 10, borderRadius: 10, alignItems: "center", marginTop: 10 },

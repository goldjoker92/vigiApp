...
const buildEndereco = (a) => {
  ...
  if (full1 && typeof full1 === 'string' && full1.trim()) {
    return full1.trim();
  }
...
async function fetchAlertaFirestore(alertId) {
  ...
  if (!snap.exists()) {
    console.warn('[public-alerts/[id].jsx] Firestore: aucun document pour id:', alertId);
    return null;
  }
...
export default function TelaAlertaPublica() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('[public-alerts/[id].jsx] ERREUR fetch:', e?.message || e);
      } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (status !== 'granted') {
          return console.warn('[public-alerts/[id].jsx] geo: permission refusée');
        }
        ...
        console.error('[public-alerts/[id].jsx] geo error:', e?.message || e);
      }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    if (u && i) {
      ...
      console.log('[public-alerts/[id].jsx] distance non calculée — user:', u, ' inc:', i);
    }
  }, [
    locUsuario,
    alerta.coords,
    ...
    <SafeAreaView style={estilos.container}>
      ...
      >
        ...
        >
          ...
          {regiao ? (
            <View style={estilos.mapWrap}>
              <MapView
                ...
                onMapReady={() => console.log('[public-alerts/[id].jsx] carte: prête')}
              >
                <Marker coordinate={regiao} title={alerta.tipo || 'Incidente'} />
                {coordUsuario && (
                  <Marker
              ...
              <Pressable
                ...
                style={({ pressed }) => [estilos.recenterBtn, pressed && { opacity: 0.85 }]}
              >
            ...
            <View style={estilos.banner}>
              ...
              <Text style={estilos.bannerText}>Localização indisponível para este alerta.</Text>
            </View>
          )}

          {/* Détails */}
          <View style={estilos.card}>
            <Linha label="🚨 Tipo" valor={alerta.tipo} cor={CORES.critico} compact />
            <Linha
              label="📍 Endereço"
              ...
              extraGap={scale(12)}
            />
            <Linha label="🏙️ Cidade" valor={alerta.cidade} cor={CORES.vigi} compact />
            <Linha label="🗺️ Estado" valor={alerta.estado} cor={CORES.neutro} compact />
            <Linha
              label="📏 Distância (evento ~ usuário)"
              valor={
                coordUsuario && coordIncidente ? distanciaTexto(coordUsuario, coordIncidente) : null
              }
              valorNode={coordUsuario && coordIncidente ? null : <LoaderDots />}
              cor={CORES.atencao}
              compact
            />
            <Linha
          ...
          {/* Description */}
          <View style={estilos.card}>
            <Text style={[estilos.cardTitle, { color: CORES.critico }]}>Descrição</Text>
...
function Linha({ label, valor, valorNode, cor, multiline = false, compact = false, extraGap = 0 }) {
  return (
    <View style={[estilos.row, compact && estilos.rowCompact]}>
      <Text style={[estilos.rowLabel, { color: cor }]} numberOfLines={3} ellipsizeMode="clip">
        {label}
      </Text>

      {valorNode ? (
        <View style={{ flex: 1, alignItems: 'flex-end', marginLeft: extraGap }}>{valorNode}</View>
      ) : (
        <Text
          style={[
            estilos.rowValue,

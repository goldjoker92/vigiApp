...
function hasAnyRegex(t, arr) {
  const n = norm(t);
  return arr.some((k) => new RegExp(`\\b${norm(k)}\\b`, 'i').test(n));
}
...
function parseVictimCount(text = '') {
  ...
  for (const w in words) {
    if (
      new RegExp(`\\b${w}\\b\\s+(vitim|victim|vitimas|victimes|feridos?|pessoas?|personnes?)`).test(
        n,
      )
    ) {
      return words[w];
...
export function extractTextFeatures(desc = '') {
  ...
  let violence = 0;
  if (hasWeapon) {
    violence = /tiro|tiros|disparo|arma de fogo|tirs/.test(t) ? 3 : 2;
  } else if (isBareHands) {
    violence = 1;
  }

  const V = LEX.victims;
  const victim = hasAnyRegex(t, V.baby)
    ? 'baby'
    : hasAnyRegex(t, V.child)
      ? 'child'
      : hasAnyRegex(t, V.woman)
        ? 'woman'
        : hasAnyRegex(t, V.elderly)
          ? 'elderly'
          : hasAnyRegex(t, V.man)
            ? 'man'
            : hasAnyRegex(t, V.animal)
              ? 'animal'
              : hasAnyRegex(t, V.property)
                ? 'property'
                : hasAnyRegex(t, V.shop)
                  ? 'shop'
                  : 'generic';
...
export function featuresSimilarity(a, b) {
  ...
  const dv = Math.abs((a.violence || 0) - (b.violence || 0));
  s += dv === 0 ? 0.12 : dv === 1 ? 0.03 : -0.1;
  const va = a.victimCount || 0,
    vb = b.victimCount || 0;
  if (va === vb && va > 0) {
    s += 0.12;
  } else if (va > 0 || vb > 0) {
    s += Math.abs(va - vb) === 1 ? 0.03 : -0.08;
  }
  if (a.victim === b.victim) {
    s += ['baby', 'child', 'elderly', 'woman'].includes(a.victim) ? 0.16 : 0.09;
  } else {
    const clash = (x, y) => (x === 'shop' && y === 'woman') || (x === 'woman' && y === 'shop');
    s += clash(a.victim, b.victim) ? -0.15 : -0.05;
  }
  if (a.isTrafficIncident && b.isTrafficIncident) {
    s += 0.15;
    if (va + vb >= 2) {
      s += 0.06;
    }
  } else if (a.isTrafficIncident !== b.isTrafficIncident) {
    s -= 0.18;
  }
  if (a.isDrowning && b.isDrowning) {
    s += 0.2;
  }
  if (a.isFracture && b.isFracture) {
    s += 0.15;
  }
  if (a.highFootfall === b.highFootfall) {
    s += 0.06;
  } else {
    s -= 0.03;
  }
  if (a.isAggression && b.isAggression) {
    s += 0.1;
  }
  if (a.isFire && b.isFire) {
    s += 0.16;
  }
  if ((a.isAggression && b.isFire) || (a.isFire && b.isAggression)) {
    s -= 0.3;
  }
...
export function timeBucketKey(date = new Date(), windowMin = 60) {
  ...
  d.setMinutes(Math.floor(d.getMinutes() / windowMin) * windowMin, 0, 0);
...
export function spatialBucketKey(lat, lng, km = 1) {
  ...
  const GRID_LAT = DEG_PER_KM_LAT * km;
  const GRID_LNG = GRID_LAT / Math.cos((lat * Math.PI) / 180);
  const latBucket = Math.round(lat / GRID_LAT);
  const lngBucket = Math.round(lng / GRID_LNG);
...
function normalizeAggressive(s = '') {
  let out = String(s || '')
    ...
    .replace(/[^\S\r\n]+/g, ' ')
    .toLowerCase();
  out = out.replace(/./g, (ch) => L33T_MAP[ch] ?? HOMO_MAP[ch] ?? ch);
  return out;
}
function tightForm(s = '') {
  return normalizeAggressive(s)
    .replace(/[^a-z0-9]+/g, '')
    .replace(/([a-z0-9])\1{2,}/g, '$1$1');
}
function flexRegexFromWord(word) {
  const w = tightForm(word);
  const letters = w.split('').map((ch) => ch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const body = letters.join('[^a-z0-9]{0,3}');
...
function matchAnyForbidden(text = '', extraAliases = []) {
  ...
  const nAgg = normalizeAggressive(text);
  const tight = tightForm(text);
  const matches = new Set();
  for (const raw of ALL) {
    const t = tightForm(raw);
    if (t && tight.includes(t)) {
      matches.add(raw);
    }
  }
  for (const raw of ALL) {
    const re = flexRegexFromWord(raw);
    if (re.test(nAgg)) {
      matches.add(raw);
    }
  }
  const tokens = normalizeAggressive(text)
    .split(/[^a-z0-9]+/)
    .filter(Boolean);
  const SHORTS = new Set(['cv', 'pc', 'pf', 'pm', 'pcc']);
  tokens.forEach((tok) => {
    if (tok.length <= 4 && SHORTS.has(tok)) {
      matches.add(tok.toUpperCase());
    }
  });
  return [...matches];
}
...
export function maskKnownPlacesForForbidden(text = '', places = []) {
  if (!text || !Array.isArray(places) || places.length === 0) {
    return text;
  }
